...
const CartSummary = ({ guests, activeGuestId, totalPrice, onContinue, showContinue, onRemoveService, onRemoveAll, onChangeStaff, onChangeTime, onChangeService, currentStep, onBook, isBooking, bookingResult, bookingErrors, user }) => {
    ...
    const handleOverlayTouchStart = (e) => { dragStartY.current = e.touches[0].clientY; setIsDragging(true); };
    const handleOverlayTouchMove = (e) => {
        ...
        if (delta > 0) {
            ...
            animationRef.current = requestAnimationFrame(() => {
                if (overlayRef.current) { overlayRef.current.style.transform = `translateY(${delta}px)`; overlayRef.current.style.transition = 'none'; }
            });
    ...
    const handleOverlayTouchEnd = () => {
        ...
        if (finalDelta > 200) setIsReviewOpen(false);
        else if (overlayRef.current) { overlayRef.current.style.transform = ''; overlayRef.current.style.transition = 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)'; }
    ...
    const formatServiceName = (name) => name ? name.replace(/\s*\([\d\s\w]+\)\s*/gi, '').trim() : "";
    ...
    const validationMsg = (currentStep === 1 && missingServices) ? `Please select a service for ${missingServices.name}`
        : (currentStep === 1 && missingMainService) ? `Please select a main service for ${missingMainService.name}`
        : (currentStep === 2 && missingStaff) ? `Please select a professional for ${missingStaff.name}`
        : (currentStep === 3 && missingTime) ? `Please select a time for ${missingTime.name}`
        : null;
    ...
    return (
        <>
            <div ref={overlayRef} className={`${styles.container} ${isReviewOpen ? styles.overlayOpen : ''}`}>
                ...
                {!hasServices ? <div className={styles.emptyState}><p>No services selected yet</p></div> : (
                    <div className={styles.cartContent}>
                        <div className={styles.cartHeader}>
                            <div className={styles.cartTitleWrapper}>
                                <div className={styles.cartIconWrapper}><FiCalendar className={styles.cartIcon} size={22} /><span className={styles.badge}>{itemCount}</span></div>
                        ...
                        <div className={styles.scrollArea}>
                            {guests.map((guest) => guest.services.length > 0 && (
                                <div key={guest.id} className={styles.guestGroup}>
                                    <div className={styles.guestName}>
                                        <span>{guest.name}</span>
                                        {guest.staff && <button className={styles.guestStaff} onClick={() => { onChangeStaff(guest.id); setIsReviewOpen(false); }}>{guest.staff.id === 'any' ? 'Any professional' : `with ${guest.staff.name}`} ✎</button>}
                                        {guest.time && <button className={styles.guestTime} onClick={() => { onChangeTime(guest.id); setIsReviewOpen(false); }}>🕐 {guest.time.time} ✎</button>}
                                    </div>
                                                ...
                                                <li key={`${guest.id}-${item.id}-${index}`} className={styles.item}>
                                                    <div className={styles.itemMain}>
                                                        <div className={styles.itemInfo}>
                                                            <span className={styles.itemName}>{item.isOvertime ? '🌙 ' : ''}{formatServiceName(item.name)}</span>
                                                            <div className={styles.itemMeta}><span>{item.duration}</span><span className={styles.itemDot}>•</span><span>${item.price}</span></div>
                                                        </div>
                                                        <div className={styles.itemAction}>
                                                            {!item.isOvertime && <button className={styles.removeSingleBtn} onClick={() => onRemoveService(guest.id, item.id)}><FiTrash2 size={16} /></button>}
                                                        </div>
                                                    </div>
                                                </li>
                                            ))}
                ...
                <div className={styles.footer}>
                    ...
                    {hasServices && showContinue && (
                        <div className={styles.buttonWrapper}>
                            ...
                            <button className={`${styles.continueBtn} ${isDisabled ? styles.disabledBtn : ''}`} onClick={() => !isDisabled && onContinue()}>{validationMsg && isDisabled ? 'Select Details' : 'Continue'}</button>
                        </div>
                    )}
                    {allHaveTime && (
                        <div className={styles.reviewFooter}>
                            <button onClick={onBook} disabled={isBooking} className={styles.compactBookBtn}>
                                {isBooking ? 'Processing...' : (user && user.name ? `Book as ${user.name}` : 'Complete Booking')}
                            </button>
                        </div>
                    )}
                </div>
            ...
            {hasServices && !isReviewOpen && (
                <div ref={floatingBarRef} className={`${styles.floatingBar} ${isDisabled ? styles.floatingBarDisabled : ''}`} onClick={() => setIsReviewOpen(true)} onTouchStart={handleBarTouchStart} onTouchEnd={handleBarTouchEnd}>
                    <div className={styles.floatingInfo}>
                        <div className={styles.pullUpIndicator}><FiChevronUp className={styles.pullUpIcon} /></div>
                        <div className={styles.floatingText}>
                            <span className={styles.floatingTitle}>{validationMsg && isDisabled ? validationMsg : 'Your Booking'}</span>
                            <span className={styles.floatingDetails}>{itemCount} items • <strong>${totalPrice}</strong></span>
                        </div>
                    </div>
                    {showContinue ? (
                        <button className={`${styles.floatingContinueBtn} ${isDisabled ? styles.floatingDisabledBtn : ''}`} onClick={(e) => { e.stopPropagation(); if (!isDisabled) onContinue(); }}>Continue</button>
                    ) : (allHaveTime && (
                        <button className={styles.floatingBookBtn} onClick={(e) => { e.stopPropagation(); onBook(); }} disabled={isBooking}>{isBooking ? '...' : 'Complete'}</button>
                    ))}
                </div>
            )}
